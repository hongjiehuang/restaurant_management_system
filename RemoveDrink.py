...
class removedrink:
    def __init__(self):
        ...
        self.drinkL = Label(self.mainframe, text="Drink NO: ")
    ...
    def okayE(self, event):
        removable=False
        cursor = QF.cursor
        query = "SELECT NO FROM Beverages2;"
        ...
        if removable==True:
            QF.cursor.callproc('remove_drink', something)
            QF.cnx.commit()
            messagebox.showinfo("Success", "Congratulation!\nYou have successfully removed a drink.")
            self.window.destroy()
        else:
