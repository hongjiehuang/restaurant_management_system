...
class addfood:
    def __init__(self):
        ...
        self.ingredientsL.grid(row = 2, column = 0)
        self.foodnameL.grid(row = 1, column = 0)
        self.priceL.grid(row=3, column=0)
        self.popularityL.grid(row=4, column=0)

        self.ingredientsI.grid(row = 2, column = 1)
        self.foodnameI.grid(row = 1, column = 1)
        self.priceI.grid(row=3, column=1)
        self.popularityI.grid(row=4, column=1)
        self.popularityI.insert(0,0)
    ...
    def okayE(self, event):
        ...
        try:
            if float(price) < 0 or int(popularity) < 0:
                messagebox.showerror("Error", "The price and quantity should be positive!")
                return
            ...
            QF.cursor.callproc('add_food', food)
            QF.cnx.commit()
            messagebox.showinfo("Success", "Congratulation!\nYou have successfully added a food.")
            self.window.destroy()
        except:
